...
class DynamoDBCachingService(DynamoDBBase, CacheService):
    ...
    def get(self, question: str) -> str | None:
        ...
        if similar_question:
            ...
            if 'Item' in response:
                logger.info(f"Found similar question in cache: '{similar_question}' for input: '{question}' with chat_id: '{self.chat_id}'")
    ...
    def find_similar_question(self, question: str, question_embedding: list[float]) -> str | None:
        current_time = int(time.time())
        cutoff_time = current_time - self.cache_time_interval

        filter_expression = Attr('timestamp').gt(cutoff_time) & Attr('chat_id').eq(self.chat_id)
        ...
        for item in items:
            ...
            if similarity >= self.cache_similarity_threshold and similarity > max_similarity:
                max_similarity = similarity
                most_similar_question = item['question']

        if most_similar_question:
            logger.debug(f"Found similar question in cache: '{most_similar_question}' for input: '{question}' with chat_id: '{self.chat_id}'")
        else:
            logger.debug(f"No similar question found in cache for input: '{question}' with chat_id: '{self.chat_id}'")
    ...
    def evict_old_entries(self):
        ...
        if (total_items >= self.max_cache_size):
            ...
            items_to_remove = total_items - self.max_cache_size + 1  # +1 to make room for the new item
            logger.debug(f"Cache size limit reached. Evicting {items_to_remove} oldest entries.")
            ...
            items_to_delete = sorted(items, key=lambda x: x['timestamp'])[:items_to_remove]

            # Delete the oldest items
            with self.table.batch_writer() as batch:
                for item in items_to_delete:
                    batch.delete_item(Key={'question': item['question'], 'chat_id': item['chat_id']})
