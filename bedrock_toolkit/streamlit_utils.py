...
log_level_options = Literal[
    "TRACE", "DEBUG", "INFO", "SUCCESS", "WARNING", "ERROR", "CRITICAL"
]


def write_stream(
    stream: Generator[str | dict[str, Any], None, None],
...
def print_response_data(
    ...
    if "tool_calls" in response_data:
        ...
        for tool_call in response_data["tool_calls"]:
            print_func(
                f"{tool_call.__class__.__name__}: {tool_call.model_dump_json(indent=json_indent)}"
            )
        ...
        for message in response_data["messages"]:
            ...
            for content in message["content"]:
                if "text" in content:
                    ...
                    print_func(
                        f"Tool Use: {json.dumps(content['toolUse'], indent=json_indent)}"
                    )
                elif "toolResult" in content:
                    print_func(
                        f"Tool Result: {json.dumps(content['toolResult'], indent=json_indent)}"
                    )
...
def display_model_response(
    streamed_text: str, response_data: dict[str, Any] | None
) -> None:
    """
    ...
    if response_data:
        ...
        if "messages" in response_data:
            create_expandable_section(
                "Message counts", count_message_types(response_data["messages"])
            )
            create_expandable_section("Messages", response_data["messages"])

        if "tool_calls" in response_data:
            create_expandable_section(
                "Tool calls in last response", response_data["tool_calls"]
            )

        if "usage" in response_data:
            create_expandable_section(
                "Token usage in last response", response_data["usage"]
            )
...
def create_sidebar_options(
    ...
    options["model_id"] = str(selected_model)

    options["use_streaming"] = bool(st.sidebar.checkbox("Use Streaming", value=True))

    options["max_retries"] = int(
        st.sidebar.number_input("Max Retries", min_value=1, max_value=10, value=3)
    )

    invoke_limit_input = st.sidebar.number_input(
        "Invoke Limit", min_value=1, max_value=20, value=default_invoke_limit
    )
    options["invoke_limit"] = (
        int(invoke_limit_input) if invoke_limit_input is not None else None
    )
