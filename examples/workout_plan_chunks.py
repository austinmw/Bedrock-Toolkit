...
def get_workout_plan_class(num_days: int):
    ...
    class WorkoutDay(BaseModel):
        description: str = Field(
            description="General description of the workout for this day."
        )
        duration: int = Field(
            ge=0,
            description="Duration of the workout in minutes. Every day requires a duration. Rest days should have a duration of 0.",
        )
        focus_area: str = Field(
            description="Primary focus area or activity type (e.g., upper body, running)."
        )
        exercises: List[str] = Field(
            default_factory=list,
            description="List of key exercises for the day in the format 'Exercise Name: Sets x Reps', 'Exercise Name: Time', or 'Exercise Name: Time x Distance', etc.",
        )
...
def main() -> None:
    ...
    with st.form("user_input"):
        ...
        height_in = st.number_input(
            "Height (inches)", min_value=0, max_value=11, value=11
        )
        body_fat = st.number_input(
            "Body Fat Percentage", min_value=5, max_value=70, value=17
        )
        fitness_level = st.selectbox(
            "Fitness Level", ["Beginner", "Intermediate", "Advanced"], index=1
        )
        goals = st.text_input(
            "Fitness goals", value="Get to 12% bodyfat without dropping below 162 lbs"
        )
    ...
    if submit_button:
        ...
        for chunk in range(chunks):
            ...
            # Update tool manager for this chunk
            st.session_state.conversation.model_runner.tool_manager = ToolManager(
                tool_schemas, tool_processors
            )
            ...
            with assistant_placeholder:
                with st.spinner(
                    f"Generating your personalized workout plan for days {start_day}-{end_day}..."
                ):
                    try:
                        # Call the model and process the output
                        streamed_text, response_data = (
                            st.session_state.conversation.say(
                                text=user_prompt,
                        ...
                        for item in response_data["tool_calls"]:
                            if isinstance(item, WorkoutPlan):
                                st.session_state.workout_dataframe = (
                                    update_workout_dataframe(
                                        item,
                                        start_day,
                                        st.session_state.workout_dataframe,
                                    )
                        ...
                        st.error(
                            "An error occurred. Consider increasing the max retries."
                        )
                        st.exception(e)
